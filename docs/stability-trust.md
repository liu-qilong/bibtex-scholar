...
|-----------|--------|--------|
| Cite decorations rebuild only when caret enters/leaves a cite span | `src/core/cite-span.ts` | `tests/cite-span.test.ts` |
| Chip visibility filter is pure half-open policy (`spans_showing_chips`); multi-cite lines hide only the span under the caret | `src/core/cite-span.ts` | `tests/cite-span.test.ts` |
...
| Failed cache persist keeps the coalescer dirty so a later flush retries (no silent drop) | `src/infra/save-coalesce.ts` | `tests/save-coalesce.test.ts` |
| `citation_popup.dispose` drops active subscribers (no ghost CardManager listeners after unload/remount) | `src/ui/citation-popup.ts` | `tests/citation-popup.test.ts` |
| Unknown Reading-view cites are visual-only (dashed underline + tooltip), not a Notice per key | `src/main.ts` + `src/ui/ux-copy.ts` | `tests/ux-copy.test.ts` |
| Paint-time duplicate blocks: one summary Notice, source tag “not cached”, first-wins copy | `src/ui/ux-copy.ts` + codeblock processor | `tests/ux-copy.test.ts` |
...
| Vault rescan: first id + first DOI wins; pure rebuild | `src/core/cache-ops.ts` | `tests/cache-ops.test.ts` |
...
| Frequent saves coalesce (not one write per codeblock entry) | `src/infra/save-coalesce.ts` | `tests/save-coalesce.test.ts` |
| Popup debounce / grace / single open / ESC suppress / dispose | `src/ui/citation-popup.ts` | `tests/citation-popup.test.ts` |
| Clash grouping undirected + reason merge | `find_clashes` | `tests/find-clashes.test.ts` |
| DOI clash checks O(1) via `doi_index` | `src/architecture/doi-index.ts` | `tests/doi-index.test.ts` |
| Citekey duplicate/clash checks and inline-cite resolution are case-insensitive; storage/display keep the user's literal casing | `src/architecture/citekey-index.ts` | `tests/citekey-index.test.ts`, `tests/bibtex.test.ts`, `tests/find-clashes.test.ts`, `tests/vault-scan.test.ts` |
...
| Pinned cards: dedup by paper id, detach from the anchor/hover-close machinery entirely, survive what would otherwise close a preview card (click-outside, scroll, mouseleave grace); Esc closes only the front-most pin, one at a time; unpin/× always closes (no "revert to hover-tracked" state) | `src/ui/pin-registry.ts`, `src/hover.tsx` (`CardShell`, `CardManager`) | `tests/pin-registry.test.ts`, `tests/hover-popup.test.tsx` |
| Rename vault scan is chunked, prioritised, cancelable | `src/scale/vault-scan.ts` | `tests/vault-scan.test.ts` |
...
| Idle = no popup + no dirty save + no rename timers | `src/infra/idle-audit.ts` | `tests/idle-audit.test.ts` |
| Paper panel discover/list views: random sampling capped/deduped/rng-injectable; list mode's filter+sort is unbounded (no mount cap, virtualized); mention-count sort is descending with an alpha tiebreak | `src/scale/library-scale.ts` (`random_sample_ids`, `filtered_ids`, `compare_by_mention_count`) | `tests/library-scale.test.ts` |
| `cite_index` can be warmed independent of any rename target (`old_id` optional); O(1) mention count accessor distinct from the display-oriented path-list accessor | `src/scale/vault-scan.ts` (`scan_inline_cites_chunked`, `cite_index_count_for`) | `tests/vault-scan.test.ts` |
...
| Directory BibTeX export: matches by real path segment (`"notes/"` prefix), not a same-prefixed sibling folder (`"notes-archive/"`); union of entries sourced under the folder *and* entries cited by any note under it even when sourced elsewhere; renders only the requested ids, sorted | `src/core/cache-ops.ts` (`ids_under_path`, `format_bibtex_for_ids`), `src/scale/vault-scan.ts` (`cite_index_all_cites`), `src/main.ts` (`export_directory_bibtex`) | `tests/cache-ops.test.ts`, `tests/vault-scan.test.ts` |
...
| `Debouncer` is a clock-injectable trailing debounce (same convention as `CitationPopupController`'s `PopupClock`): rapid triggers coalesce into one call after the delay; `cancel()` before the deadline never fires | `src/scale/debounce.ts` | `tests/debounce.test.ts` |
| `should_repaint_window` keys on both `{start, end}` and a list-identity token, so a stale cached range from a previous query/sort can never suppress a real repaint (a fresh list always gets a new identity token); `diff_window_ids` only reports ids that actually entered/left a window | `src/scale/library-scale.ts` | `tests/library-scale.test.ts` |
...
| Panel search box | 130ms trailing debounce (immediate on query-clear) coalesces rapid keystrokes into one scan + repaint | `src/scale/debounce.ts` (`Debouncer`), `panel.ts` |
